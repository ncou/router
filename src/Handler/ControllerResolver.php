...
//https://github.com/symfony/http-kernel/blob/master/Controller/ControllerResolver.php
class ControllerResolver
{
    public function getController($controller, ServerRequestInterface $request)
    {
        // TODO : récupérer le controller dans la request et si le controller ou l'action est null lever une exception !!!!
        ...
        if (\is_array($controller)) {
            ...
            if (!\is_callable($controller)) {
                throw new \InvalidArgumentException(sprintf('The controller for URI "%s" is not callable. %s', $request->getUri()->getPath(), $this->getControllerError($controller)));
            }
            return $controller;
        }
        ...
        try {
            ...
            throw new \InvalidArgumentException(sprintf('The controller for URI "%s" is not callable. %s', $request->getUri()->getPath(), $e->getMessage()));
        }

        if (!\is_callable($callable)) {
            throw new \InvalidArgumentException(sprintf('The controller for URI "%s" is not callable. %s', $request->getUri()->getPath(), $this->getControllerError($callable)));
        }
        return $callable;
    }
